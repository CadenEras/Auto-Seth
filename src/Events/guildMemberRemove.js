...
module.exports = new Event("guildMemberRemove", (client, member) => {
	...
	if (!kickLog) return channelDev.send(`${member.user.tag} left the guild by their own will.`);
    ...
    try{
        if (target.id === member.id) {
            channelDev.send(`${member.user.tag} left the guild; kicked by ${executor.tag}, reason : ${reason}`);
        } else {
            channelDev.send(`${member.user.tag} left the guild or was kicked, audit log fetch was inconclusive.`);
        }
    }catch(error){
        console.log(error)
