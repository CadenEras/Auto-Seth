   ...
   async run(message, args, client) {
      try {
         const embed1 = new Discord.MessageEmbed()
            ...
            .addField('Premium count :', `${message.guild.premiumSubscriptionCount}`)
            .addField('Verification level :', `${message.guild.verificationLevel}`)
            .addField('NSFW level :', `${message.guild.nsfwLevel}`)
            .addField('Explicit content filter level :', `${message.guild.explicitContentFilter}`)
            .addField('Verified :', `${message.guild.verified}`)
            .addField('System channel :', `${message.guild.systemChannel} (${message.guild.systemChannelId})`)
